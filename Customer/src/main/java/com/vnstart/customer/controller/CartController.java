...
public class CartController {
    ...
    public String cart(Model model, Principal principal, HttpSession session) {
        ...
        if(shoppingCart.getTotalItems()==0){
            model.addAttribute("check","Chưa có sản phẩm trong giỏ hàng!!!");

        }
        session.setAttribute("totalItems",shoppingCart.getCartItem());
        model.addAttribute("subTotal",shoppingCart.getTotalItems());
        model.addAttribute("shoppingCart",shoppingCart);
        System.out.println(shoppingCart.getCartItem());
    ...
    public String deleteItemFromCart(@RequestParam("productId") Long productId,
                                     Model model,
                                     Principal principal){
